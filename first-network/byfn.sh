...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.testnet.svc.cluster.local/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.testnet.svc.cluster.local/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.testnet.svc.cluster.local | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.testnet.svc.cluster.local
    docker cp -a orderer.testnet.svc.cluster.local:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.testnet.svc.cluster.local
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.testnet.svc.cluster.local

    for PEER in peer0.org1.testnet.svc.cluster.local peer1.org1.testnet.svc.cluster.local peer0.org2.testnet.svc.cluster.local peer1.org2.testnet.svc.cluster.local; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.testnet.svc.cluster.local/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.testnet.svc.cluster.local/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.testnet.svc.cluster.local`` & ``peer0.org2.testnet.svc.cluster.local``).  Secondly, we point to
